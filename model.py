...
class PredictionConvolutions(nn.Module):
    ...
    def __init__(self, n_classes):
        ...
        # 这里乘4是因为每个先验框需要预测4个偏移量（位置坐标相对于先验框的偏移量）
        self.loc_conv = nn.Conv2d(512, n_boxes * 4, kernel_size=3, padding=1)

        # Class prediction convolutions (predict classes in localization boxes)
        # 注意这里的n_classes应该等于你要预测的类别数+1(background)
    ...
    def init_conv2d(self):
        ...
        for c in self.children():
            if isinstance(c, nn.Conv2d):
                nn.init.xavier_uniform_(c.weight)  # xavier是一种初始化的方式
    ...
    def forward(self, pool5_feats):
        ...
        locs = l_conv.view(batch_size, -1, 4)  # (N, 441, 4), there are a total 441 boxes on this feature map ；441=7*7*9
        ...
        return locs, classes_scores  # locs[N,441,4]  class_scores[N,441,20]
...
class tiny_detector(nn.Module):
    ...
    def create_prior_boxes(self):
        ...
        """
        fmap_dims = 7  # feature map的尺寸7*7
        obj_scales = [0.2, 0.4, 0.6]  # 框的尺寸
    ...
    def detect_objects(self, predicted_locs, predicted_scores, min_score, max_overlap, top_k):
        ...
        :param min_score: minimum threshold for a box to be considered a match for a certain class 大于这个min_score这个阈值就能将其视为某一类
        :param max_overlap: maximum overlap two boxes can have so that the one with the lower score is not suppressed via NMS 超过这个max_overlap就会被NMS删掉
        ...
        for i in range(batch_size):
            ...
            for c in range(1, self.n_classes):
                ...
                class_scores = predicted_scores[i][:, c]  # (441) 第c类别目标在441个锚框的信赖值
                score_above_min_score = class_scores > min_score  # torch.uint8 (byte) tensor, for indexing
                n_above_min_score = score_above_min_score.sum().item()  # c类别中锚框置信度大于设定阈值的个数
                if n_above_min_score == 0:
                    continue
                class_scores = class_scores[
                    score_above_min_score]  # (n_qualified), n_min_score <= 441 挑选出class_scores中符合要求的锚框
                class_decoded_locs = decoded_locs[score_above_min_score]  # (n_qualified, 4) 对符合要求的锚框解码得到预测的边界框

                # Sort predicted boxes and scores by scores
                class_scores, sort_ind = class_scores.sort(dim=0,
                                                           descending=True)  # (n_qualified), (n_min_score) # 对符合要求的置信度进行排序并得到对应锚框的index
                class_decoded_locs = class_decoded_locs[
                    sort_ind]  # (n_min_score, 4) 按顺序排列这几个边界框[n_above_min_score,4],4为预测边界框的坐标
                ...
                for box in range(class_decoded_locs.size(0)):
                    # If this box is already marked for suppression 如果有想删掉的框，可在suppress矩阵中标注为1
                    if suppress[box] == 1:
                        continue
                ...
                image_labels.append(
                    torch.LongTensor((1 - suppress).sum().item() * [c]).to(device))  # 留下来的框对应位置是1，再*c，则标签就表示是c类
                image_scores.append(class_scores[1 - suppress])  # 对应位置的置信度就是该类别的置信度
...
class MultiBoxLoss(nn.Module):
    ...
    def __init__(self, priors_cxcy, threshold=0.5, neg_pos_ratio=3, alpha=1.):
        ...
        self.priors_cxcy = priors_cxcy  # [cx,cy,size*ratio,size/ratio] size:(n_boxes,4)
        self.priors_xy = cxcy_to_xy(priors_cxcy)  # [x_min,y_min,x_max,y_max] size:(n_boxes,4)
        self.threshold = threshold  # 阈值
        self.neg_pos_ratio = neg_pos_ratio
        self.alpha = alpha

        self.smooth_l1 = nn.L1Loss()
        self.cross_entropy = nn.CrossEntropyLoss(reduce=False)  # reduce=False时，输出了每一个锚框的loss
    ...
    def forward(self, predicted_locs, predicted_scores, boxes, labels):
        ...
        for i in range(batch_size):
            ...
            overlap = find_jaccard_overlap(boxes[i], self.priors_xy)  # (n_objects, 441) 真实边界框和锚框之间的IOU值

            # For each prior, find the object that has the maximum overlap
            overlap_for_each_prior, object_for_each_prior = overlap.max(
                dim=0)  # (441) IOU和对应的index，这个找到的是与每个锚框IOU最大的真实框
            # overlap_for_each_prior对overlap的第0维进行取最大值，第0维是n_objects,即取每个锚框（列）对应的真实框（行）的最大值
            # object_for_each_prior这个（441）对应的是每个锚框所对应的真实框在第几行，假如说第n个锚框对应的最大IOU是第m个目标，那么object_for_each_prior第n个数就是m

            # We don't want a situation where an object is not represented in our positive (non-background) priors -
            # 不想出现一个物体没有在我们非背景的先验框中出现的情况
            # 1. An object might not be the best object for all priors, and is therefore not in object_for_each_prior.
            # 2. All priors with the object may be assigned as background based on the threshold (0.5).

            # To remedy this -
            # First, find the prior that has the maximum overlap for each object.
            _, prior_for_each_object = overlap.max(dim=1)  # (N_o) 找到与真实框框IOU最大的锚框 （n_objects）
            '''这个就是找到每一行（真实框）对应的每一列（锚框）的最大值；
            比如若prior_for_each_object为[197,206],则表示第1个真实框所对应的最大IOU值的锚框为第197个锚框，第二个真实框对应第206个锚框'''

            # Then, assign each object to the corresponding maximum-overlap-prior. 把每个目标分给对应的最大IOU值的锚框(This fixes 1.)
            object_for_each_prior[prior_for_each_object] = torch.LongTensor(range(n_objects)).to(device)
            '''
               因为已经选择了真实框对应的最大锚框，那么就要在object_for_each_prior中对应的锚框位置的目标改成其对应的真实框，假如说
               prior_for_each_objects为[3,71,78,197],那么就要在object_for_each_prior中第3个锚框对应的真实框定为0，第71个
               定为1，第78个定为2，第197个定为3，注意：object_for_each_prior中第1个数对应的是第0个锚框！
            '''

            # To ensure these priors qualify, artificially give them an overlap of greater than 0.5. (This fixes 2.)
            overlap_for_each_prior[
                prior_for_each_object] = 1.  # 将真实框所对应的IOU最大的锚框在每个框对应的Overlap矩阵中置为1，防止因为其对应的最大的IOU值小于阈值而漏检
            # 这里相当于把上面例子中overlap_for_each_prior中第197个和第206个数置为1，因为这是真实目标所对应的锚框，即使不大于阈值，我们也要把它留下来

            # Labels for each prior
            label_for_each_prior = labels[i][object_for_each_prior]  # (441) 第i张图片
            '''
            这里根据object_for_each_prior中的对应的真实框来设定label_for_each_prior的值
            例如:若labels[i]输出为[14,14,15,15],因为object_for_each_prior中每个位置对应的值只能是0,1,2,3；因为n_objects，
            则将object_for_each_prior中0对应为label_for_each_prior中的14, 1对应14,2对应15,3对应15，即这一步是将第几个真实框直接转换成label
            '''

            # Set priors whose overlaps with objects are less than the threshold to be background (no object)
            label_for_each_prior[overlap_for_each_prior < self.threshold] = 0  # (441)
            # 这里将小于阈值label置为0,0对应背景

            # Store
            true_classes[i] = label_for_each_prior  # 将label信息保存起来

            # Encode center-size object coordinates into the form we regressed predicted boxes to
            true_locs[i] = cxcy_to_gcxgcy(xy_to_cxcy(boxes[i][object_for_each_prior]), self.priors_cxcy)  # (441, 4)

        # 从这里已经退出batch_size的循环了，注意有些变量就多了个batch_size维
        ...
        n_positives = positive_priors.sum(dim=1)  # (N)batch_size中每张图片的441个锚框中对应的positive_prior的数量

        n_hard_negatives = self.neg_pos_ratio * n_positives  # (N)
        # First, find the loss for all priors ; true_classes [N,441] predicted_scores [N,441,n_classes]
        ...
        # print('hardness_ranks:', hardness_ranks.shape, hardness_ranks)
